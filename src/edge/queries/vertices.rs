...
impl Edge {
    ...
    pub fn has_vertex_with_attr_str_key(&self, attr_k: &str) -> bool {
        self.get_from_vertex().has_attr_str_key_equals_to(attr_k)
            || self.get_to_vertex().has_attr_str_key_equals_to(attr_k)
    }

    /// Checks if "from" or "to" vertex has any attribute like
    pub fn has_vertex_with_attr_key_like(&self, attr_k: &str) -> bool {
        self.get_from_vertex().has_attr_key_like(attr_k)
            || self.get_to_vertex().has_attr_key_like(attr_k)
    }

    /// Checks if "from" or "to" vertex has a string attribute key like
    pub fn has_vertex_with_attr_str_key_like(&self, attr_k: &str) -> bool {
        self.get_from_vertex().has_attr_str_key_like(attr_k)
            || self.get_to_vertex().has_attr_str_key_like(attr_k)
    }
    ...
    pub fn has_vertex_with_attr_vec_u8_key(&self, attr_k: &str) -> bool {
        self.get_from_vertex().has_attr_vec_u8_key_equals_to(attr_k)
            || self.get_to_vertex().has_attr_vec_u8_key_equals_to(attr_k)
    }
    ...
    pub fn has_vertex_with_attr_vec_u8_key_like(&self, attr_k: &str) -> bool {
        self.get_from_vertex().has_attr_vec_u8_key_like(attr_k)
            || self.get_to_vertex().has_attr_vec_u8_key_like(attr_k)
    }

    /// Checks if "from" or "to" vertex has a Vec<u8> attribute and equal for value
    pub fn has_vertex_with_attr_vec_u8_value_equals_to(
        &self,
        attr_k: &str,
        attr_v: &Vec<u8>,
    ) -> bool {
